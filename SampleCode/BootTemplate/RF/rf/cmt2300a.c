...
BOOL CMT2300A_AutoSwitchStatus(u8 nGoCmd)
{
    ...
    u8 nTout=100; //for 10 mS timeout
	  u8  nWaitStatus;
...
void CMT2300A_SetNodeID(u8 id)
{	
 CMT2300A_WriteReg(CMT2300A_CUS_PKT20, id);
}
/*! ********************************************************
...
void CMT2300A_Init(void)
{
    ...
    CMT2300A_SoftReset();
    for(tmp=0;tmp<20;tmp++) CMT2300A_DelayUs(1000);  //20 mS delay for start
